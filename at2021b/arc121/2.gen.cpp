...
using VI=vec<int>;using VC=vec<char>;using VPII=vec<pair<int, int>>;
#line 1 "alias.hpp"//2b
...
void solve() {
  ...
  */
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  N: "<<(N)ln<<"  A: "<<(A)ln<<"  C: "<<(C)ln;}
  VPII R, G, B;
  times(N*2, i) {
    if(C[i] == 'R') R.PB({A[i], i});
    if(C[i] == 'G') G.PB({A[i], i});
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  R: "<<(R)ln<<"  G: "<<(G)ln<<"  B: "<<(B)ln;}
  ...
  int ans2 = LLONG_MAX / 2;
  {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  rbs: "<<(rbs)ln<<"  gbs: "<<(gbs)ln;}
    times(size(rbs), i) {
      int jj = -1;
      times(size(gbs), j) {
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  ans1: "<<(ans1)ln<<"  ans2: "<<(ans2)ln;}
