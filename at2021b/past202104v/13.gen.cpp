...
struct SegTree {
  ...
  void update(int a, int b, int v, int k, int l, int r) {
    if(r <= a || b <= l) return;
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  a:                "<<(a)ln<<"  b:                "<<(b)ln<<"  v:                "<<(v)ln<<"  k:                "<<(k)ln<<"  l:                "<<(l)ln<<"  r:                "<<(r)ln<<"  a <= l && r <= b: "<<(a <= l && r <= b)ln<<"  same[k]:          "<<(same[k])ln<<"  val[k]:           "<<(val[k])ln;}
    if(a <= l && r <= b) {
      if(same[k]) {
        int xx = r - l;
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  count[val[k]]: "<<(count[val[k]])ln<<"  count[v]:      "<<(count[v])ln<<"  nPair:         "<<(nPair)ln<<"  xx:            "<<(xx)ln;}
        ...
        count[v] += xx;
        nPair += count[v] * (count[v] - 1) / 2;
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  count[val[k]]: "<<(count[val[k]])ln<<"  count[v]:      "<<(count[v])ln<<"  nPair:         "<<(nPair)ln;}
      } else {
