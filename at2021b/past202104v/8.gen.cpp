...
void solve() {
  ...
  */
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  P:  "<<(P)ln<<"  SP: "<<(SP)ln<<"  C:  "<<(C)ln<<"  SC: "<<(SC)ln<<"  D:  "<<(D)ln<<"  SD: "<<(SD)ln;}
  int ans = LLONG_MAX;
  upto(0, (M + K - 1) / K, b) {
    int temp = b * Q;
    ...
    if(cc == 0) {
      temp += SP[M];
      {if(debug)cerr<<'#'<<__LINE__ ln<<"  b:    "<<(b)ln<<"  cc:   "<<(cc)ln<<"  temp: "<<(temp)ln;}
    } else {
      int th = C[cc - 1];
      int dd = distance(begin(D), leftmost_gt(iter(D), th /*, greater<int>()*/));
      if(cc + dd > M) {
        temp += SC[cc] + SD[M - cc];
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  b:        "<<(b)ln<<"  cc:       "<<(cc)ln<<"  dd:       "<<(dd)ln<<"  SC[cc]:   "<<(SC[cc])ln<<"  SD[M-cc]: "<<(SD[M-cc])ln<<"  temp:     "<<(temp)ln;}
      } else {
        temp += SP[M];
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  b:     "<<(b)ln<<"  cc:    "<<(cc)ln<<"  dd:    "<<(dd)ln<<"  SP[M]: "<<(SP[M])ln<<"  temp:  "<<(temp)ln;}
      }
    }
    amin(ans, temp);
  }
