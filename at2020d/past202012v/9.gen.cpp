...
using VI=vec<int>;using VTIII=vec<tuple<int, int, int>>;
#line 1 "alias.hpp"//9b
...
void solve() {
  ...
  */
  ...
  times(M, i) {
    ...
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  a:          "<<(a)ln<<"  b:          "<<(b)ln<<"  ans[b]:     "<<(ans[b])ln<<"  ans[a] + 1: "<<(ans[a] + 1)ln;}
    amin(ans[b], ans[a] + 1);
  }
  times(N, i) {
