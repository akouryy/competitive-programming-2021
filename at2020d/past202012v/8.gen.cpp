...
using VS=vec<string>;using WB=vvec<bool>;using VB=vec<bool>;using VPII=vec<pair<int, int>>;
#line 1 "alias.hpp"//8b
...
AC[]="AC",ac[]="ac",WA[]="WA",wa[]="wa",ERROR[]="ERROR",error[]="error",
  ...
  while(!next.empty()) {
    ...
    times(4, d) {
      ...
      if(0 <= ii && ii < H && 0 <= jj && jj < W) {
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  i:               "<<(i)ln<<"  j:               "<<(j)ln<<"  d:               "<<(d)ln<<"  ii:              "<<(ii)ln<<"  jj:              "<<(jj)ln<<"  visited[ii][jj]: "<<(visited[ii][jj])ln<<"  S[ii][jj]:       "<<(S[ii][jj])ln<<"  \"v<^>\"[d]:     "<<("v<^>"[d])ln;}
