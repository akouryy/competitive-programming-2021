...
struct SegTree {
  ...
  int node[N*2];
  SegTree() { times(N*2, i) node[i] = LLONG_MIN/2 ; }
  /** 0-based **/
  ...
  int query(int l, int r) const {
    ...
    int a = LLONG_MIN/2 ;
    while(l < r) {
      if(l & 1) a =  max(a, node[l]);
      if(r & 1) a =  max(node[r&-2], a);
      l = (l+1) >> 1;
      r >>= 1;
    }
    return a;
...
void solve() {
  ...
  */
  ...
  {if(debug)cerr<<"0: "<<(0)ln;}
  times(N-1, i) {
    {if(debug)cerr<<"i: "<<(i)ln;}
    pos_left.update(i, L[i]);
    neg_right.update(i, -R[i]);
  }
  {if(debug)cerr<<"1: "<<(1)ln;}
  times(Q, i) {
    int a = A[i];
    int lng = -1, lok = B[i];
    while(abs(lng - lok) > 1) {
      int m = (lng + lok + 1) / 2;
      {if(debug)cerr<<'#'<<__LINE__ ln<<"  m:                         "<<(m)ln<<"  pos_left.query(m, B[i]):   "<<(pos_left.query(m, B[i]))ln<<"  a:                         "<<(a)ln<<"  -neg_right.query(m, B[i]): "<<(-neg_right.query(m, B[i]))ln;}
      if(pos_left.query(m, B[i]) <= a && a <= -neg_right.query(m, B[i])) {
        lok = m;
      } else {
        lng = m;
      }
    ...
    while(abs(rng - rok) > 1) {
      int m = (rng + rok) / 2;
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  m:                         "<<(m)ln<<"  pos_left.query(B[i], m):   "<<(pos_left.query(B[i], m))ln<<"  a:                         "<<(a)ln<<"  -neg_right.query(B[i], m): "<<(-neg_right.query(B[i], m))ln;}
      if(pos_left.query(B[i], m) <= a && a <= -neg_right.query(B[i], m)) {
        rok = m;
      } else {
        rng = m;
      }
    }
    {if(debug)cerr<<'#'<<__LINE__ ln<<"  A[i]: "<<(A[i])ln<<"  lok:  "<<(lok)ln<<"  B[i]: "<<(B[i])ln<<"  rok:  "<<(rok)ln;}
    cout << rok - lok + 1 ln;
  }
