...
using WB=vvec<bool>;using VB=vec<bool>;using VI=vec<int>;using VWI=vec<vvec<int>>;
#line 1 "alias.hpp"//1b
...
void solve() {
  ...
  */
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  okp:   "<<(okp)ln<<"  okq:   "<<(okq)ln<<"  okr:   "<<(okr)ln<<"  okpq:  "<<(okpq)ln<<"  okpqr: "<<(okpqr)ln;}
  VI dp(N+1);
  upto(0, N, i) {
    upto(i, N, j) {
  ...
  {if(debug)cerr<<'#'<<__LINE__ ln<<"  pq: "<<(pq)ln<<"  pr: "<<(pr)ln;}
  if(pq && pr) {
    for(char c : S) {
      if(c == T[0]) {
        ++ans;
