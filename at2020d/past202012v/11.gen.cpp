...
using VS=vec<string>;using VD=vec<LD>;using VPII=vec<pair<int, int>>;
#line 1 "alias.hpp"//1b
  ...
  uptil(1, 1<<16, b) if((b & ~ans_index) == 0) {
    times(4, ci) times(4, cj) {
      ...
      times(4, d) {
        ...
        if(0 <= ii && ii < 4 && 0 <= jj && jj < 4 && (b >> ii*4+jj) & 1) {
          ps.PB({ii, jj});
        }
      }
      int n = size(ps);
      if(n) {
        ...
        {if(debug)cerr<<'#'<<__LINE__ ln<<"  b:             "<<(b)ln<<"  ci:            "<<(ci)ln<<"  cj:            "<<(cj)ln<<"  ps:            "<<(ps)ln<<"  sum:           "<<(sum)ln<<"  n:             "<<(n)ln<<"  sum/n + 5.0/n: "<<(sum/n + 5.0/n)ln;}
